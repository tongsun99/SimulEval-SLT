...
class SignInstance(Instance):
    ...
    def preprocess_source(self, source):
        ...
        # self.audio_info = soundfile.info(source)
        # self.sample_rate = self.audio_info.samplerate
        self.sample_rate = int(source.split()[-1])
        self.samples = []
        return source
    ...
    def send_src(self, segment_size=10):
        if self.step == 0:
            self.start_time = time.time()
            self.load_sign_from_fni(self.source)   # fni: 'file name index' 
        assert segment_size >= 1, "instance size has to larger than 1 ms"
        ...
        if self.step < len(self.samples):
            ...
            # Finish reading this sign tensor
            dict_to_return = {
                "segment_id": self.source_length(),
                "segment": DEFAULT_EOS,
                "sample_rate": self.sample_rate,
    ...
    def load_sign_from_fni(self, file_name_index):
        file, name, index, _ = file_name_index.split(' ')
        index = int(index)
        ...
        assert tmp[index]["name"] == name
        self.samples = tmp[index]["sign"].tolist() # (T, D)
    ...
    def source_info(self):
        return str({
            "sample_rate": self.sample_rate,
            "samples length": len(self.samples),    # length
            "source length": self.source_length(),  # time
        })
